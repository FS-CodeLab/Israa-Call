...
function showInstallButton() {
    ...
    if (installContainer) {
        installContainer.innerHTML = `
            <button onclick="installApp()" class="bg-emerald-600 hover:bg-emerald-700 text-white px-3.5 py-2 rounded-xl text-xs font-bold shadow-md transition flex items-center gap-1.5 animate-bounce">
                📲 تثبيت التطبيق
            </button>
        `;
    }
...
async function installApp() {
    ...
    if (deferredPrompt) {
        ...
        if (outcome === 'accepted') {
            ...
            if (typeof showToast === 'function') showToast('تم تثبيت التطبيق بنجاح على جهازك 📲');
        }
        deferredPrompt = null;
    }
}
...
setInterval(() => { checkForUpdates(false); }, 1000 * 60 * 30);